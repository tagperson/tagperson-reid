...
class AugMix(object):
    ...
    def __init__(self, prob=0.5, aug_prob_coeff=0.1, mixture_width=3, mixture_depth=1, aug_severity=1, cfg=None):
        """
        Args:
    ...
    def __call__(self, image):
        ...
        # may need to random something
        if self.cfg.INPUT.AUGMIX.AUG_SEVERITY_RANDOM.ENABLED:
            aug_severity_min = self.cfg.INPUT.AUGMIX.AUG_SEVERITY_RANDOM.MIN
            aug_severity_max = self.cfg.INPUT.AUGMIX.AUG_SEVERITY_RANDOM.MAX
            self.aug_severity = random.randint(aug_severity_min, aug_severity_max)
        if self.cfg.INPUT.AUGMIX.MIXTURE_DEPTH_RANDOM.ENABLED:
            mixture_depth_min = self.cfg.INPUT.AUGMIX.MIXTURE_DEPTH_RANDOM.MIN
            mixture_depth_max = self.cfg.INPUT.AUGMIX.MIXTURE_DEPTH_RANDOM.MAX
            self.mixture_depth = random.randint(mixture_depth_min, mixture_depth_max)
        if self.cfg.INPUT.AUGMIX.AUG_PROB_COEFF > 0:
            self.aug_prob_coeff = self.cfg.INPUT.AUGMIX.AUG_PROB_COEFF
