...
model_urls = {
    ...
    'ibn_b_50x': 'https://github.com/XingangPan/IBN-Net/releases/download/v1.0/resnet50_ibn_b-9ca61e85.pth',
}
...
class BasicBlock(nn.Module):
    ...
    def __init__(self, inplanes, planes, bn_norm, with_ibn=False, with_ibn_b=False, with_se=False,
        ...
        self.IN = nn.InstanceNorm2d(planes, affine=True) if with_ibn_b else None
...
class Bottleneck(nn.Module):
    ...
    def __init__(self, inplanes, planes, bn_norm, with_ibn=False, with_ibn_b=False, with_se=False,
        ...
        self.IN = nn.InstanceNorm2d(planes * 4, affine=True) if with_ibn_b else None
        self.relu = nn.ReLU(inplace=True)
        if with_se:
...
class ResNet(nn.Module):
    def __init__(self, last_stride, bn_norm, with_ibn, with_ibn_b, with_se, with_nl, block, layers, non_layers):
        self.inplanes = 64
        super().__init__()
        ...
        self.layer1 = self._make_layer(block, 64, layers[0], 1, bn_norm, with_ibn, with_ibn_b, with_se)
        self.layer2 = self._make_layer(block, 128, layers[1], 2, bn_norm, with_ibn, with_ibn_b, with_se)
        # self.layer3 = self._make_layer(block, 256, layers[2], 2, bn_norm, with_ibn, False, with_se)
        self.layer3 = self._make_layer(block, 256, layers[2], 2, bn_norm, with_ibn, with_ibn_b, with_se)
    ...
    def _make_layer(self, block, planes, blocks, stride=1, bn_norm="BN", with_ibn=False, with_ibn_b=False, with_se=False):
        downsample = None
        ...
        layers = []
        # layers.append(block(self.inplanes, planes, bn_norm, with_ibn, False, with_se, stride, downsample))
        layers.append(block(self.inplanes, planes, bn_norm, with_ibn, with_ibn_b, with_se, stride, downsample))
        self.inplanes = planes * block.expansion
        for i in range(1, blocks):
            # layers.append(block(self.inplanes, planes, bn_norm, with_ibn, False if (i < blocks-1) else with_ibn_b, with_se))
            layers.append(block(self.inplanes, planes, bn_norm, with_ibn, with_ibn_b, with_se))
...
def build_resnet_backbone(cfg):
    ...
    with_ibn_b    = cfg.MODEL.BACKBONE.WITH_IBN_B
    ...
    model = ResNet(last_stride, bn_norm, with_ibn, with_ibn_b, with_se, with_nl, block,
    ...
    if pretrain:
        ...
        if pretrain_path:
            ...
            if with_ibn_b: key = 'ibn_b_' + key
            if with_se:  key = 'se_' + key

            state_dict = init_pretrained_weights(key)

        # change key if neccessary
        if 'model' in state_dict.keys():
            middle_state_dict = {}
            for k, v in state_dict['model'].items():
                m_k = k.replace("backbone.", "")
                middle_state_dict[m_k] = v
            state_dict = middle_state_dict
